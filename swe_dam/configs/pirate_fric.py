...
def get_config():
    ...
    config.dataset = "radial_dam_break_data_inter_t41_xy161.npz"
    # config.dataset = "radial_dam_break_data_inter_t41_xy41.npz"

    # Weights & Biases
    config.wandb = wandb = ml_collections.ConfigDict()
    wandb.project = "PINN-SWE-DAM"
    wandb.name = "pirate_cf_fric"
    wandb.tag = None
    wandb.notes = "cuteflow, 1 tw, 50k, 2048, nl=8, sfx, no pi_init"
    ...
    transfer.s2s_pi_init = False # Leave as True if s2s_transfer is also True. Will change to false after first window.
